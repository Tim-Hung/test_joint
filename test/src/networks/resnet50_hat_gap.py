...
class Net(torch.nn.Module):
    ...
    def __init__(self,inputsize,taskcla):
        ...
        self.last=torch.nn.Linear(2048,2)#100

        self.gap=GlobalAvgPool2d()

        self.gate=torch.nn.Sigmoid()
        em_word = 1#len(self.taskcla)#1
    ...
    #def forward(self,x):
        ...
        """
        em_id=torch.autograd.Variable(torch.LongTensor([0]).cuda(),volatile=False)#t=0
        #print('t in mask should be 0: ',t)
        ...
        #gc1=self.gate(s*self.ec1(em_id)*self.gap(h))
        """
        ...
        """
        gc1=self.gate(s*self.ec1(em_id)*torch.sum(self.gap(h), 0, True))
        #print('self.gap(h)',self.gap(h),'sum',torch.sum(self.gap(h), 0, True))
        #print('h1.size()',h.size(),'gc1.size()',gc1.size(),'h*gc1.view(1,-1,1,1)')#,(h*gc1.view(1,-1,1,1)).size())#,'self.gap(h).size()',self.gap(h).size(),'gc1.size()',gc1.size())
        h=h*gc1.view(1,-1,1,1).expand_as(h)
        #layer2-1
        identity=h
        h=self.relu(self.bn2_1_1(self.c2_1_1(h)))
        gc2_1_1=self.gate(s*self.ec2_1_1(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc2_1_1.view(1,-1,1,1).expand_as(h);
        h=self.relu(self.bn2_1_2(self.c2_1_2(h)))
        gc2_1_2=self.gate(s*self.ec2_1_2(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc2_1_2.view(1,-1,1,1).expand_as(h);
        h=self.bn2_1_3(self.c2_1_3(h))
        gc2_1_3=self.gate(s*self.ec2_1_3(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc2_1_3.view(1,-1,1,1).expand_as(h)

        identity=self.bn2_1_d(self.c2_1_d(identity))
        gc2_1_d=self.gate(s*self.ec2_1_d(em_id)*torch.sum(self.gap(identity), 0, True))
        identity=identity*gc2_1_d.view(1,-1,1,1).expand_as(identity)

        h+=identity
        gct2_1=self.gate(s*self.ect2_1(em_id)*torch.sum(self.gap(h), 0, True))
        h=self.relu(h);h=h*gct2_1.view(1,-1,1,1).expand_as(h)
        #2-2
        identity=h
        h=self.relu(self.bn2_2_1(self.c2_2_1(h)))
        gc2_2_1=self.gate(s*self.ec2_2_1(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc2_2_1.view(1,-1,1,1).expand_as(h);
        h=self.relu(self.bn2_2_2(self.c2_2_2(h)))
        gc2_2_2=self.gate(s*self.ec2_2_2(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc2_2_2.view(1,-1,1,1).expand_as(h);
        h=self.bn2_2_3(self.c2_2_3(h))
        gc2_2_3=self.gate(s*self.ec2_2_3(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc2_2_3.view(1,-1,1,1).expand_as(h)

        h+=identity;h=self.relu(h)
        #2-3
        identity=h
        h=self.relu(self.bn2_3_1(self.c2_3_1(h)))
        gc2_3_1=self.gate(s*self.ec2_3_1(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc2_3_1.view(1,-1,1,1).expand_as(h);
        h=self.relu(self.bn2_3_2(self.c2_3_2(h)))
        gc2_3_2=self.gate(s*self.ec2_3_2(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc2_3_2.view(1,-1,1,1).expand_as(h);
        h=self.bn2_3_3(self.c2_3_3(h))
        gc2_3_3=self.gate(s*self.ec2_3_3(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc2_3_3.view(1,-1,1,1).expand_as(h)

        h+=identity;h=self.relu(h)
        #3-1
        identity=h
        h=self.relu(self.bn3_1_1(self.c3_1_1(h)))
        gc3_1_1=self.gate(s*self.ec3_1_1(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc3_1_1.view(1,-1,1,1).expand_as(h);
        h=self.relu(self.bn3_1_2(self.c3_1_2(h)))
        gc3_1_2=self.gate(s*self.ec3_1_2(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc3_1_2.view(1,-1,1,1).expand_as(h);
        h=self.bn3_1_3(self.c3_1_3(h))
        gc3_1_3=self.gate(s*self.ec3_1_3(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc3_1_3.view(1,-1,1,1).expand_as(h)

        identity=self.bn3_1_d(self.c3_1_d(identity))
        gc3_1_d=self.gate(s*self.ec3_1_d(em_id)*torch.sum(self.gap(h), 0, True))
        identity=identity*gc3_1_d.view(1,-1,1,1).expand_as(identity)#;print('+1l+',identity.shape)

        h+=identity
        gct3_1=self.gate(s*self.ect3_1(em_id)*torch.sum(self.gap(h), 0, True))
        h=self.relu(h);h=h*gct3_1.view(1,-1,1,1).expand_as(h)
        #3-2
        identity=h
        h=self.relu(self.bn3_2_1(self.c3_2_1(h)))
        gc3_2_1=self.gate(s*self.ec3_2_1(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc3_2_1.view(1,-1,1,1).expand_as(h)
        h=self.relu(self.bn3_2_2(self.c3_2_2(h)))
        gc3_2_2=self.gate(s*self.ec3_2_2(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc3_2_2.view(1,-1,1,1).expand_as(h)
        h=self.bn3_2_3(self.c3_2_3(h))
        gc3_2_3=self.gate(s*self.ec3_2_3(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc3_2_3.view(1,-1,1,1).expand_as(h)

        h+=identity;h=self.relu(h)
        #3-3
        identity=h
        h=self.relu(self.bn3_3_1(self.c3_3_1(h)))
        gc3_3_1=self.gate(s*self.ec3_3_1(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc3_3_1.view(1,-1,1,1).expand_as(h);
        h=self.relu(self.bn3_3_2(self.c3_3_2(h)))
        gc3_3_2=self.gate(s*self.ec3_3_2(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc3_3_2.view(1,-1,1,1).expand_as(h);
        h=self.bn3_3_3(self.c3_3_3(h))
        gc3_3_3=self.gate(s*self.ec3_3_3(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc3_3_3.view(1,-1,1,1).expand_as(h)

        h+=identity;h=self.relu(h)
        #3-4
        identity=h
        h=self.relu(self.bn3_4_1(self.c3_4_1(h)))
        gc3_4_1=self.gate(s*self.ec3_4_1(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc3_4_1.view(1,-1,1,1).expand_as(h);
        h=self.relu(self.bn3_4_2(self.c3_4_2(h)))
        gc3_4_2=self.gate(s*self.ec3_4_2(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc3_4_2.view(1,-1,1,1).expand_as(h);
        h=self.bn3_4_3(self.c3_4_3(h))
        gc3_4_3=self.gate(s*self.ec3_4_3(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc3_4_3.view(1,-1,1,1).expand_as(h)

        h+=identity;h=self.relu(h)
        
        #4-1
        identity=h
        h=self.relu(self.bn4_1_1(self.c4_1_1(h)))
        gc4_1_1=self.gate(s*self.ec4_1_1(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc4_1_1.view(1,-1,1,1).expand_as(h)
        h=self.relu(self.bn4_1_2(self.c4_1_2(h)))
        gc4_1_2=self.gate(s*self.ec4_1_2(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc4_1_2.view(1,-1,1,1).expand_as(h)
        h=self.bn4_1_3(self.c4_1_3(h))
        gc4_1_3=self.gate(s*self.ec4_1_3(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc4_1_3.view(1,-1,1,1).expand_as(h)

        identity=self.bn4_1_d(self.c4_1_d(identity))
        gc4_1_d=self.gate(s*self.ec4_1_d(em_id)*torch.sum(self.gap(h), 0, True))
        identity=identity*gc4_1_d.view(1,-1,1,1).expand_as(identity)#;print('+1l+',identity.shape)

        h+=identity
        gct4_1=self.gate(s*self.ect4_1(em_id)*torch.sum(self.gap(h), 0, True))
        h=self.relu(h);h=h*gct4_1.view(1,-1,1,1).expand_as(h)
        #4-2
        identity=h
        h=self.relu(self.bn4_2_1(self.c4_2_1(h)))
        gc4_2_1=self.gate(s*self.ec4_2_1(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc4_2_1.view(1,-1,1,1).expand_as(h);
        h=self.relu(self.bn4_2_2(self.c4_2_2(h)))
        gc4_2_2=self.gate(s*self.ec4_2_2(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc4_2_2.view(1,-1,1,1).expand_as(h)
        h=self.bn4_2_3(self.c4_2_3(h))
        gc4_2_3=self.gate(s*self.ec4_2_3(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc4_2_3.view(1,-1,1,1).expand_as(h)

        h+=identity;h=self.relu(h)
        #4-3
        identity=h
        h=self.relu(self.bn4_3_1(self.c4_3_1(h)))
        gc4_3_1=self.gate(s*self.ec4_3_1(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc4_3_1.view(1,-1,1,1).expand_as(h);
        h=self.relu(self.bn4_3_2(self.c4_3_2(h)))
        gc4_3_2=self.gate(s*self.ec4_3_2(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc4_3_2.view(1,-1,1,1).expand_as(h)
        h=self.bn4_3_3(self.c4_3_3(h))
        gc4_3_3=self.gate(s*self.ec4_3_3(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc4_3_3.view(1,-1,1,1).expand_as(h)

        h+=identity;h=self.relu(h)
        #4-4
        identity=h
        h=self.relu(self.bn4_4_1(self.c4_4_1(h)))
        gc4_4_1=self.gate(s*self.ec4_4_1(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc4_4_1.view(1,-1,1,1).expand_as(h);
        h=self.relu(self.bn4_4_2(self.c4_4_2(h)))
        gc4_4_2=self.gate(s*self.ec4_4_2(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc4_4_2.view(1,-1,1,1).expand_as(h)
        h=self.bn4_4_3(self.c4_4_3(h))
        gc4_4_3=self.gate(s*self.ec4_4_3(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc4_4_3.view(1,-1,1,1).expand_as(h)

        h+=identity;h=self.relu(h)
        #4-5
        identity=h
        h=self.relu(self.bn4_5_1(self.c4_5_1(h)))
        gc4_5_1=self.gate(s*self.ec4_5_1(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc4_5_1.view(1,-1,1,1).expand_as(h)
        h=self.relu(self.bn4_5_2(self.c4_5_2(h)))
        gc4_5_2=self.gate(s*self.ec4_5_2(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc4_5_2.view(1,-1,1,1).expand_as(h)
        h=self.bn4_5_3(self.c4_5_3(h))
        gc4_5_3=self.gate(s*self.ec4_5_3(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc4_5_3.view(1,-1,1,1).expand_as(h)

        h+=identity;h=self.relu(h)

        #4-6
        identity=h
        h=self.relu(self.bn4_6_1(self.c4_6_1(h)))
        gc4_6_1=self.gate(s*self.ec4_6_1(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc4_6_1.view(1,-1,1,1).expand_as(h);
        h=self.relu(self.bn4_6_2(self.c4_6_2(h)))
        gc4_6_2=self.gate(s*self.ec4_6_2(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc4_6_2.view(1,-1,1,1).expand_as(h)
        h=self.bn4_6_3(self.c4_6_3(h))
        gc4_6_3=self.gate(s*self.ec4_6_3(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc4_6_3.view(1,-1,1,1).expand_as(h)

        h+=identity;h=self.relu(h)

        #5-1
        identity=h
        h=self.relu(self.bn5_1_1(self.c5_1_1(h)))
        gc5_1_1=self.gate(s*self.ec5_1_1(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc5_1_1.view(1,-1,1,1).expand_as(h);
        h=self.relu(self.bn5_1_2(self.c5_1_2(h)))
        gc5_1_2=self.gate(s*self.ec5_1_2(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc5_1_2.view(1,-1,1,1).expand_as(h);
        h=self.bn5_1_3(self.c5_1_3(h))
        gc5_1_3=self.gate(s*self.ec5_1_3(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc5_1_3.view(1,-1,1,1).expand_as(h)

        identity=self.bn5_1_d(self.c5_1_d(identity))
        gc5_1_d=self.gate(s*self.ec5_1_d(em_id)*torch.sum(self.gap(h), 0, True))
        identity=identity*gc5_1_d.view(1,-1,1,1).expand_as(identity)

        h+=identity
        gct5_1=self.gate(s*self.ect5_1(em_id)*torch.sum(self.gap(h), 0, True))
        h=self.relu(h);h=h*gct5_1.view(1,-1,1,1).expand_as(h)
        #5-2
        identity=h
        h=self.relu(self.bn5_2_1(self.c5_2_1(h)))
        gc5_2_1=self.gate(s*self.ec5_2_1(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc5_2_1.view(1,-1,1,1).expand_as(h);
        h=self.relu(self.bn5_2_2(self.c5_2_2(h)))
        gc5_2_2=self.gate(s*self.ec5_2_2(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc5_2_2.view(1,-1,1,1).expand_as(h);
        h=self.bn5_2_3(self.c5_2_3(h))
        gc5_2_3=self.gate(s*self.ec5_2_3(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc5_2_3.view(1,-1,1,1).expand_as(h)

        h+=identity;h=self.relu(h)
        #5-3
        identity=h
        h=self.relu(self.bn5_3_1(self.c5_3_1(h)))
        gc5_3_1=self.gate(s*self.ec5_3_1(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc5_3_1.view(1,-1,1,1).expand_as(h);
        h=self.relu(self.bn5_3_2(self.c5_3_2(h)))
        gc5_3_2=self.gate(s*self.ec5_3_2(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc5_3_2.view(1,-1,1,1).expand_as(h)
        h=self.bn5_3_3(self.c5_3_3(h))
        gc5_3_3=self.gate(s*self.ec5_3_3(em_id)*torch.sum(self.gap(h), 0, True))
        h=h*gc5_3_3.view(1,-1,1,1).expand_as(h)
        ...
        masks = gc1,gc2_1_1,gc2_1_2,gc2_1_3,gc2_1_d,gct2_1,gc2_2_1,gc2_2_2,gc2_2_3,gc2_3_1,gc2_3_2,gc2_3_3,\
        gc3_1_1,gc3_1_2,gc3_1_3,gc3_1_d,gct3_1,gc3_2_1,gc3_2_2,gc3_2_3,gc3_3_1,gc3_3_2,gc3_3_3,gc3_4_1,gc3_4_2,gc3_4_3,\
        gc4_1_1,gc4_1_2,gc4_1_3,gc4_1_d,gct4_1,gc4_2_1,gc4_2_2,gc4_2_3,gc4_3_1,gc4_3_2,gc4_3_3,gc4_4_1,gc4_4_2,gc4_4_3,\
        gc4_5_1,gc4_5_2,gc4_5_3,gc4_6_1,gc4_6_2,gc4_6_3,\
